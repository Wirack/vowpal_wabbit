...
namespace recall_tree_ns
{
		...
		example& ec)
	{
		...
		bool storeScores = b.all->vm.count("scores");
		//Init array with zero scores, to make vw_prediction.cpp interface work
		if (storeScores) {
			ec.pred.scalars = {};
			for (int i = 0; i < b.k; i++) {
				ec.pred.scalars.push_back(0.f);
			}
		}
			...
			++ls)
		{
			...
			if (storeScores) {
				ec.pred.scalars[ls->label] = ec.partial_prediction;
			}
